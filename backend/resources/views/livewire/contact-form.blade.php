<form wire:submit="submit" class="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
    <div>
        <label for="name" class="mb-2 block text-sm text-slate-200">{{ __('portfolio.form.name') }}</label>
        <input
            id="name"
            type="text"
    ...
    <div>
        <label for="email" class="mb-2 block text-sm text-slate-200">{{ __('portfolio.form.email') }}</label>
        <input
            id="email"
            type="email"
    ...
    <div>
        <label for="message" class="mb-2 block text-sm text-slate-200">{{ __('portfolio.form.message') }}</label>
        <textarea
            id="message"
            rows="5"
    ...
    @if ($sent)
        <p class="rounded-lg border border-emerald-600/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-200">
            {{ __('portfolio.form.sent_success') }}
        </p>
    @endif
