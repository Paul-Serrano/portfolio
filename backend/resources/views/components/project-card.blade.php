...
<article class="group rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-900 to-slate-900/70 p-6 transition hover:-translate-y-1 hover:border-violet-400/40">
    <div class="mb-3 flex items-center justify-between gap-4">
        ...
        @if (!empty($project['featured']))
            <span class="rounded-full bg-violet-500/15 px-3 py-1 text-xs font-medium text-violet-200">Featured</span>
        @endif
    </div>
    ...
    <div class="mt-4 grid gap-3 text-sm text-slate-300">
        <p><span class="font-semibold text-slate-100">Challenge:</span> {{ $project['challenge'] }}</p>
        <p><span class="font-semibold text-slate-100">Outcome:</span> {{ $project['outcome'] }}</p>
    </div>
    ...
    <div class="mt-6 flex flex-wrap gap-3">
        ...
        @if (!empty($project['demo']))
            <x-cta-button :href="$project['demo']" target="_blank">Live Demo</x-cta-button>
        @endif
    </div>
</article>
